...
- Browser automation normally mutates state through real UI controls. The only non-UI exception is the versioned `globalThis.gravityErdAutomation` import API; keep it two-phase, fingerprint-bound, visible in the proposal dialog, and routed through the same validation and merge functions as UI imports.
